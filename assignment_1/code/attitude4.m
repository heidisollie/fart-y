...
for i = 0:N-1;
    ...
    %speed(i+1) = sqrt(v_n_b(i+1,1)^2 + v_n_b(i+1,2)^2 + v_n_b(i+1,3)^2);
    %crab_angle(i+1) = asin(v_n_b(i+1,2)/speed(i+1)) * rad2deg;
    %sideslip_angle(i+1) = asin(v_n_b(i+1,2)/speed(i+1)) * rad2deg;
    %course_angle(i+1) = (psi *rad2deg) + sideslip_angle(i+1);
end
...
for i = 0:N-1;
    ...
    v_n_b_c(i+1,:) = (R_nb * v_b_b)';
    if i == 0
        s(i+1,:) = [0 0 0];
    else
        s(i+1,:) = s(i,:) + v_n_b_c(i,:)*h;
    end
end
